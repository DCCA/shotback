...
**Design system (`src/components/ui/*` + `src/styles/globals.css` + `tailwind.config.js`):** components are driven by semantic HSL **CSS-variable tokens** (`--primary`, `--secondary`, `--muted`, `--accent`, `--destructive` + `-hover`, `--border`, `--input`, `--ring`, plus `--glow-1`/`--glow-2` for the body gradient and `--card-highlight`/`--card-shadow` for card elevation) mapped to Tailwind color utilities - use `bg-primary`/`border-input`/`ring-ring` etc., never hardcoded `emerald-*`/`slate-*`/`bg-white` literals, anywhere in `src/` (`grep -rn "slate-\|emerald-\|bg-white\|red-" src/` must stay at zero hits). **Dark applies two ways:** automatically from `@media (prefers-color-scheme: dark)` on `:root:not(.light)`, and explicitly via `class="dark"` on `<html>`; `class="light"` forces light back on. The two blocks hold the same ~28 declarations and are **deliberately duplicated** - `light-dark()` returns a colour rather than the bare HSL channels the tokens interpolate (breaking `bg-primary/40`-style alpha modifiers) and an inline pre-render script is forbidden by MV3's CSP. Both live **outside `@layer base`** so Tailwind does not tree-shake the unreferenced selectors. `tests/e2e/extension.spec.ts` guards the result: "dark theme keeps every control legible" loads the editor with `colorScheme: "dark"` and fails on any text whose luminance is within 0.3 of its painted background. `Select` is a **custom WAI-ARIA listbox** (not a native `<select>`): pass `value` + `onValueChange` + `options`, not `<option>` children — the native option popup is unstylable, which is why it was replaced.
